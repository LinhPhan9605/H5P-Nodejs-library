...
[h5p-nodejs-library](https://github.com/LinhPhan9605/h5p-nodejs-library) in a
...
framework. The **server** implements the `@LinhPhan9605/h5p-server` using
Express. You are not bound to using React or Express in your own application, as
`@LinhPhan9605/h5p-server` is framework agnostic.
...
-   The **green** parts come from one of the server-side packages of `h5p-nodejs-library`. While `@LinhPhan9605/h5p-server` is also a dev dependency of the client, this is only the case to use the TypeScript interface definitions exported by it. The dependency (which can be rather large) is only required at build time and not at runtime.
-   The **red** parts are React components that can be found in the package `@LinhPhan9605/h5p-react`. The React components wrap around the web components from `@LinhPhan9605/h5p-webcomponents`, which in turn wrap the actual core h5p player and editor JavaScript and simplify instantiation, loading, saving and event handling.
